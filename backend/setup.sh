...
DATABASE_URL="postgresql://${DB_USER}@localhost:5432/findata?schema=public"
...
createdb findata 2>/dev/null
if [ $? -eq 0 ]; then
    echo -e "${GREEN}✓ Database 'findata' created${NC}"
else
...
# Skipping Prisma generation - not configured yet
# npm run db:generate
echo -e "${GREEN}✓ Prisma setup skipped (configure as needed)${NC}"
echo ""

# Push database schema
echo "📊 Pushing database schema..."
# Skipping Prisma push - not configured yet
# npm run db:push
echo -e "${GREEN}✓ Database schema skipped (configure as needed)${NC}"
echo ""

# Seed database
echo "🌱 Seeding database with initial data..."
# Skipping seed - not configured yet
# npm run db:seed
echo -e "${GREEN}✓ Database seeding skipped (configure as needed)${NC}"
echo ""

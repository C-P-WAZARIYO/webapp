...
const setRefreshTokenCookie = (res, refreshToken) => {
  const cookieOptions = {
    httpOnly: config.cookie?.httpOnly ?? true, // Fallback if config fails
...
const register = async (req, res, next) => {
  try {
    // Add phone and emp_id here
    const { email, username, password, firstName, lastName, phone, emp_id } = req.body;
    const metadata = getRequestMetadata(req);

    const result = await authService.register(
      { email, username, password, firstName, lastName, phone, emp_id },
      metadata
    );
